...
import {
  Crown, Mail, Shield, Key, Lock, History, Save, Smartphone, Eye, EyeOff,
  Activity, CheckCircle2, UserCog,
} from 'lucide-react';
...
export function AdminProfilePage() {
  ...
  const handleSaveProfile = async () => {
    ...
    await logActivity('update_profile', 'profiles', 'Updated admin profile', fullName);
  ...
  return (
    <div className="space-y-6">
      ...
      <PageHeader title="Administrator Profile" titleKh="ប្រវត្តិអ្នកគ្រប់គ្រង" subtitle="Manage your profile, security, and activity" />
          ...
          <CardContent className="flex flex-col items-center p-6 text-center">
            <Avatar className="h-24 w-24">
              <AvatarFallback className="bg-primary/10 text-primary text-2xl font-bold">
                {(fullName || 'A').split(' ').map((n) => n[0]).slice(0, 2).join('')}
              </AvatarFallback>
            </Avatar>
            <h2 className="mt-4 text-lg font-bold">{fullName || 'Administrator'}</h2>
            <Badge variant="default" className="mt-1">
              <Crown className="mr-1 h-3 w-3" />
              {ROLE_LABELS[profile?.role ?? 'admin']?.en ?? 'Administrator'}
            </Badge>
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div className="space-y-2"><Label>New Password / ពាក្យសម្ងាត់ថ្មី</Label><Input type={showPassword ? 'text' : 'password'} value={newPassword} onChange={(e) => setNewPassword(e.target.value)} /></div>
                    <div className="space-y-2"><Label>Confirm Password / បញ្ជាក់ពាក្យសម្ងាត់</Label><Input type={showPassword ? 'text' : 'password'} value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} /></div>
                  </div>
