...
import {
  BellRing, Bell, Search, Download, Check, Trash2, BellOff, Filter, CheckCircle2,
  AlertTriangle, Info, XCircle, Users, ShieldCheck, DollarSign, Server,
} from 'lucide-react';
...
import { useAdminNotifications } from '@/hooks/use-list';
import { supabase } from '@/lib/supabase';
import { logActivity } from '@/lib/activity';
import { useAuth } from '@/hooks/use-auth';
import { formatDateTime, exportToCSV, type AdminNotification } from '@/lib/types';
import { toast } from 'sonner';

const SEVERITY_ICONS: Record<string, typeof BellRing> = {
  info: Info, warning: AlertTriangle, success: CheckCircle2, error: XCircle,
};

const TYPE_LABELS: Record<string, { en: string; kh: string; icon: typeof BellRing }> = {
  system: { en: 'System', kh: 'ប្រព័ន្ធ', icon: Server },
  approval: { en: 'Approval', kh: 'ការអនុម័ត', icon: ShieldCheck },
  member: { en: 'Member', kh: 'សមាជិក', icon: Users },
  financial: { en: 'Financial', kh: 'ហិរញ្ញវត្ថុ', icon: DollarSign },
};

export function AdminNotificationsPage() {
  const { data: notifications, setData } = useAdminNotifications();
  ...
  const memberCount = notifications.filter((n) => n.type === 'member').length;

  const handleMarkRead = async (n: AdminNotification) => {
    if (n.read) return;
    const { error } = await supabase.from('admin_notifications').update({ read: true }).eq('id', n.id);
  ...
  const handleMarkAllRead = async () => {
    ...
    await logActivity('mark_all_read', 'admin_notifications', 'Marked all as read', profile?.full_name);
  ...
  const handleExport = () => {
    exportToCSV('kot17_admin_notifications', filtered.map((n) => ({
  ...
  const columns: Column<AdminNotification>[] = [
    ...
    { key: 'type', header: 'Type', headerKh: 'ប្រភេទ', cell: (r) => {
      const t = TYPE_LABELS[r.type] ?? { en: r.type, kh: r.type };
      return <Badge variant="outline" className="text-xs">{t.en}</Badge>;
    } },
    { key: 'title', header: 'Title', headerKh: 'ចំណងជើង', cell: (r) => (
      <div><p className={r.read ? 'font-medium' : 'font-bold'}>{r.title}</p>{r.body && <p className="text-xs text-muted-foreground">{r.body}</p>}</div>
    ) },
    { key: 'status', header: 'Status', headerKh: 'ស្ថានភាព', cell: (r) => (
      <Badge variant={r.read ? 'secondary' : 'default'} className="text-xs">{r.read ? <><Check className="mr-1 h-3 w-3" />Read</> : <><BellRing className="mr-1 h-3 w-3" />Unread</>}</Badge>
    ) },
  ...
  return (
    <div className="space-y-6">
      ...
      <PageHeader
        title="Notification Center" titleKh="មជ្ឈមណ្ឌលការជូនដំណឹង"
        subtitle="System, approval, member, and financial alerts"
        actions={<>
          <Button variant="outline" onClick={handleExport}><Download className="mr-2 h-4 w-4" />Export</Button>
          <Button variant="outline" onClick={handleMarkAllRead} disabled={unreadCount === 0}><Check className="mr-2 h-4 w-4" />Mark All Read</Button>
        </>}
      />

      {/* Stats */}
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard title="Unread" titleKh="មិនទាន់អាន" value={unreadCount} icon={BellRing} accent="warning" />
        <StatCard title="System Alerts" titleKh="ប្រព័ន្ធ" value={systemCount} icon={Server} accent="info" />
        <StatCard title="Approval Alerts" titleKh="ការអនុម័ត" value={approvalCount} icon={ShieldCheck} accent="primary" />
        <StatCard title="Member Alerts" titleKh="សមាជិក" value={memberCount} icon={Users} accent="success" />
      </div>
